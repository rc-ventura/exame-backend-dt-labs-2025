...
# Fixture para o TestClient - Escopo de módulo (será executada uma vez por módulo de teste)
...
def test_register_and_get_sensor_data(client, login_user, create_server, create_sensor_data):
    token, _ = login_user
    server_ulid = create_server["ulid"]
    ...
    response = client.get(f"/data?server_ulid={server_ulid}&aggregation=hour", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
